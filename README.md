...
|---|---|---|---|
| **IdentityService** | 7151 | SQL Server | User registration, JWT token issuance |
| **DoctorService** | 7002 | SQL Server | Doctor CRUD with education, experience, awards |
| **PatientService** | 7171 | SQL Server | Patient profiles linked to Identity |
| **AppointmentService** | 7089 | SQL Server | Appointment scheduling and status |
| **BranchService** | 7118 | SQL Server | Hospital departments / branches |
| **NurseService** | 7091 | SQL Server | Nurse management |
| **PrescriptionService** | 7025 | SQL Server | Doctor prescriptions |
| **PharmacyService** | 7178 | SQL Server | Medicine catalogue with image upload |
| **MarketService** | 7241 | SQL Server + Redis | Product catalogue + Redis cart |
| **QueueService** | 7085 | SQL Server + SignalR | Patient queue with real-time hub |
| **OrderService** | 7056 | MySQL | Order management |
| **ReviewService** | 7240 | SQL Server | Patient reviews for doctors |
...
| `https://localhost:7151/swagger` | Identity API docs |
...
|---|---|---|
| `/api/auth/{everything}` | `:7151` | IdentityService |
| `/api/doctors/{everything}` | `:7002` | DoctorService |
| `/api/patients/{everything}` | `:7171` | PatientService |
| `/api/appointments/{everything}` | `:7089` | AppointmentService |
| `/api/branches/{everything}` | `:7118` | BranchService |
| `/api/nurses/{everything}` | `:7091` | NurseService |
| `/api/prescriptions/{everything}` | `:7025` | PrescriptionService |
| `/api/medicines/upload-image` | `:7178` | PharmacyService (image upload) |
| `/api/medicines/{everything}` | `:7178` | PharmacyService |
...
| `/api/queues/{everything}` | `:7085` | QueueService |
| `/api/orders/{everything}` | `:7056` | OrderService |
...
- All HTTP communication from the Web UI goes through the Ocelot gateway at `https://localhost:5000`
